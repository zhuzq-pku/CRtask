...
%[data,num,text]=xlsread(fileName);
database=table2cell(readtable([path_code filesep fileName]));
participant_label=readtable([path_code filesep subj_list])
...
for ii=1:length(subj)
    ...
    data_temp_all=[subj(ii,1),AVDR,AVDRT,NeR,NeRT,AVVR,AVVRT,NanAVDR,NanNeR,NanAVVR]
    dataall=[dataall;data_temp_all]
end

Head={'subj_name','aversive_regulate_rating','aversive_regulate_RT','Neutral_rating','Neutral_RT','aversive_view_rating','aversive_view_RT','missingAVD','missingNeutral','missingAVV','runnum'}
final=[Head;num2cell([dataall,repmat(runnum,[size(dataall,1),1])])]
Behavior_name=[path_code filesep 'ER_PM' filesep 'Behavior' filesep 'Result_Run_' num2str(runnum) '.csv' ]
