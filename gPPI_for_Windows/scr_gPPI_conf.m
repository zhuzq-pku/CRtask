...
% Jiahua Adapted for his own PC with modified from Linux/Unix version to
% Windows 10 Pro,2019
%=========================================================================%
clear
restoredefaultpath
...
%%if you have WSL in your PC with Win10 then it is ok to set to 1, JIAHUA added.

%% Set Path
spm_dir    = 'G:\[your path]\spm12_gppi';
script_dir = 'G:\[your path]\gPPI_for_Windows';

%% Please specify the data server path
paralist.data_server = 'G:\[your path]\preprocessing';

%% added by genghaiyang specify the stats server path
paralist.server_path_stats = 'G:\[your path]\First_level\First_level_Regular_2runs';
...
subjlist = fullfile(script_dir, 'Sublist_Regular_2runs_twolevel.txt');

%% Please specify the stats folder name (eg., stats_spm8)
paralist.stats_folder = '/ER/stats_spm12_swcar';

%% get ROI file list
ROI_form = 'nii';
paralist.roi_nii_folder = 'G:\[your path]\ROIs';
...
paralist.tasks_to_include = {'0', 'AvDRatOnset', 'AvRatOnset'};

%% Please specify the confound names
paralist.confound_names = {'R1', 'R2', 'R3', 'R4', 'R5', 'R6'};

%% make T contrast
Pcon.Contrasts(1).left     = {'AvDRatOnset'};
Pcon.Contrasts(1).right    = {'AvRatOnset'};
Pcon.Contrasts(1).STAT     = 'T';
Pcon.Contrasts(1).Weighted = 0;
Pcon.Contrasts(1).name     = 'Decrease';

% Pcon.Contrasts(2).left     = {'AvDRatOnset'};
% Pcon.Contrasts(2).right    = {''};
