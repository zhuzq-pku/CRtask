...
%             'fmri/stats_spm12', [stats_folder, '_gPPI']);
        ...
        subject_gPPI_stats_dir = fullfile(data_server_stats,subjects{i_subj}, ...
            'fmri/stats_spm12', [stats_folder, '_gPPI']);
...
%             subject_gPPI_stats_dir));
        ...
        % make T contrast
        P.Contrasts = Pcon.Contrasts; % Changed by Hao
        %         P.Contrasts(1).left={'InconFlk'};
        %         P.Contrasts(1).right={'ConFlk'};
        %         P.Contrasts(1).STAT='T';
        %         P.Contrasts(1).Weighted=0;
        %         %P.Contrasts(1).MinEvents=5;
        %         P.Contrasts(1).name='Alert';
...
%         unix(sprintf('/bin/rm -rf %s', '*.hdr'));
        ...
        movefile( '*.txt', ['PPI_', roi_name{i_roi}]);
       movefile( '*.mat', ['PPI_', roi_name{i_roi}]);
       movefile( '*.log', ['PPI_', roi_name{i_roi}]);
